...
namespace Microsoft.SyndicationFeed
{
    static class DateTimeUtils
    {
        ...
        private static bool TryParseDateRssSpec(string value, out DateTimeOffset result)
        {
            ...
            return DateTimeOffset.TryParseExact(wellFormattedString, parseFormat, CultureInfo.InvariantCulture.DateTimeFormat, isUtc ? DateTimeStyles.AdjustToUniversal : DateTimeStyles.None, out result);
        }
        ...
        private static bool TryParseDateRfc3339(string dateTimeString, out DateTimeOffset result)
        {
            ...
            if (dateTimeString[19] == '.')
            {
                ...
                dateTimeString = dateTimeString[..19] + dateTimeString[i..];
            }

            if (DateTimeOffset.TryParseExact(dateTimeString, Rfc3339LocalDateTimeFormat, CultureInfo.InvariantCulture.DateTimeFormat, DateTimeStyles.None, out DateTimeOffset localTime))
            {
                result = localTime;
                return true;
            }

            if (DateTimeOffset.TryParseExact(dateTimeString, Rfc3339UTCDateTimeFormat, CultureInfo.InvariantCulture.DateTimeFormat, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal, out DateTimeOffset utcTime))
            {
                result = utcTime;
                return true;
            }
