...
namespace Miniblog.Core.Services
{
	public class MetaWeblogService(
		...
		IUserServices userServices) : IMetaWeblogProvider
	{
		...
		{
			ValidateUser(username, password);
			return await blog.GetTags().Select(tag => new Tag { name = tag }).ToArrayAsync();
		}
		...
		public Task<BlogInfo[]> GetUsersBlogsAsync(string key, string username, string password)
		{
			...
			return Task.FromResult(new[] { new BlogInfo { blogid = "1", blogName = config[Constants.Config.Blog.Name] ?? nameof(MetaWeblogService), url = url } });
		}
